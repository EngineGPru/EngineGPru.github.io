...
SITE="http://enginegp.ru" # Страница проекта
GITHUB="https://github.com" # GitHub
DOMAIN="http://enginegpru.github.io" # Основной домен для работы

# GitHub
GITUSER="MaXa0H" # Логин для доступа к приватному репозиторию EngineGP
GITTOKEN="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" # Токен для доступа к приватному репозиторию EngineGP
GITLINK="https://$GITUSER:$GITTOKEN@github.com/$GITUSER/EngineGP.git" # Ссылка для клонирования репозитория
GITREQLINK="https://$GITUSER:$GITTOKEN@github.com/$GITUSER/EngineGP-requirements.git" # Ссылка для клонирования репозитория с надстройками

# Получение переменных с сервера
GAMES=$(wget -qO- $DOMAIN"/installers_variables/all" | sed -n '8p')  # Адрес репозитория игр
PHPVER=$(wget -qO- $DOMAIN"/installers_variables/all" | sed -n '11p') # Версия PHP
PMALINK=$(wget -qO- $DOMAIN"/installers_variables/all" | sed -n '23p') # Ссылка на phpMyAdmin
PMAVER=$(wget -qO- $DOMAIN"/installers_variables/all" | sed -n '20p') # Версия phpMyAdmin
SQLLINK=$(wget -qO- $DOMAIN"/installers_variables/all" | sed -n '17p') # Ссылка на MySQL
SQLVER=$(wget -qO- $DOMAIN"/installers_variables/all" | sed -n '14p') # Версия MySQL

# IPADDR=$(wget -qO- eth0.me) # Определение IP VDS
IPADDR=$(echo "${SSH_CONNECTION}" | awk '{print $3}') # Определение IP VDS (Раскомментировать первую строку в случае неработоспособности данного варианта)

VER=`cat /etc/issue.net | awk '{print $1$3}'` # Определение версии ОС
SWP=`free -m | awk '{print $2}' | tail -1` # Определение свободного места на диске
...
# Установка обязательного пакета
necPACK() {
    apt-get install -y git lsb-release apt-utils > /dev/null 2>&1
}
...
# Популярные пакеты
popPACK() {
    apt install -y pwgen dialog sudo bc lib32z1 screen htop nano tcpdump zip unzip mc lsof apt-transport-https ca-certificates safe-rm > /dev/null 2>&1
}
# Пакеты для работы панели
packPANEL() {
    apt install -y cron curl ssh nload gdb lsof qstat > /dev/null 2>&1
}
...
setMYSQL() {
    ...
    apt-get install -y ./$SQLVER.deb > /dev/null 2>&1
...
# Установка PHP
installPHP() {
    apt-get -y install php$PHPVER > /dev/null 2>&1
}
# Установка пакетов PHP
installPHPPACK() {
    apt-get install -y php$PHPVER-cli php$PHPVER-common php$PHPVER-curl php$PHPVER-mbstring php$PHPVER-mysql php$PHPVER-xml php$PHPVER-memcache php$PHPVER-memcached memcached php$PHPVER-gd php$PHPVER-zip php$PHPVER-ssh2 > /dev/null 2>&1
...
packLOCATION1() {
    apt install -y libbabeltrace1 libc6-dbg libdw1 lib32stdc++6 libreadline5 ssh qstat gdb-minimal lib32gcc1 ntpdate lsof safe-rm htop mc > /dev/null 2>&1
}
# Добавление i386
addi386() {
...
packLOCATION2() {
    apt install -y gcc-multilib > /dev/null 2>&1
}
# Настройка rclocal
setRCLOCAL() {
...
installPROFTPD() {
    ...
    apt-get install -y proftpd-mod-mysql > /dev/null 2>&1
